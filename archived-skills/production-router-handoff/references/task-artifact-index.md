...
场戏剧作任务中，`BDT-...` 场戏逐节拍表的成果类别为“任务数据集”，`DSB-...` 场戏节拍卡为“创作短卡”。BDT 的上游精确引用集合只列剧本内容／事实、当前表演母档与已采用顾问结论；DSB 精确引用当前 BDT，不允许 BDT 反向引用 DSB。BDT 内容变化先升级数据集，再升级引用它的 DSB。只有两者都为 `可交接 + 当前有效=是`、原文保护门通过且 Profile 已关闭时，才能把精确版本下传给导演调度和场戏表演适配。

场戏导演任务中，`DBD-...` 场戏调度表的成果类别为“任务数据集”，`DBC-...` 场戏调度卡为“创作短卡”。DBD 的上游精确引用集合列当前 `DSB/BDT`、表演母档、空间圣经/锚点及适用的生命体、VFX、顾问结论；DBC 只把当前 DBD 列为直接成果上游，不允许 DBD 反向引用 DBC。DBD 内容或精确引用变化时先升级数据集，再升级引用它的 DBC；只有 DBC 场戏级内容变化时可以只升级 DBC。二者都为 `可交接 + 当前有效=是`、Profile 已关闭且无调度事实阻断时，才下传；“仅专项视图生产缺口”可先交场戏表演适配，但解决并复核前不得交逐镜设计。

剪辑任务中，`ERT-...` 剪辑节奏表的成果类别为“任务数据集”，`ERC-...` 剪辑节奏卡为“创作短卡”。ERT 的上游精确引用集合列当前分镜组快照、调度、表演和场戏摄影约束；ERC 只把当前 ERT 列为直接成果上游，不允许 ERT 反向引用 ERC。顺序、时长、切点、动作/声音接点、相邻状态、生成单元或精确引用变化时先升级 ERT，再升级 ERC；只有全组摘要/Profile审计/写回切片变化时可以只升级 ERC。二者都为 `可交接 + 当前有效=是`、Profile 已关闭且无输入阻断时，才交 `shot-visual-design` 唯一写回；写回后的新分镜快照再进入逐镜摄影审核。

视频 Prompt 任务中，`VRM-...` 参考映射表的成果类别为“任务数据集”，`VPR-...` 正式 Prompt 交付卡为“Prompt”。VRM 直接引用当前分镜组、正式媒体来源或用户确认的一次性参考，不反向引用 VPR；VPR 精确引用当前 VRM 以及分镜、剪辑、摄影、表演、灯光、声音、VFX/生命体/顾问和就绪审计成果。媒体来源、顺序、平台引用名、唯一用途、允许/禁止继承、平台处理方式或适用状态变化时先升级 VRM，再升级 VPR；只改正文表达且映射完全不变时只升级 VPR。VRM 完成结构和业务复核后可交接给Prompt装配；VPR只有在用户明确放行该精确版本后才激活为 `可交接 + 当前有效=是`，这不代表生成结果可用。
