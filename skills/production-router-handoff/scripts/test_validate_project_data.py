...
class ManifestRegressionTests(unittest.TestCase):
    ...
    def sample_value(field: str, spec: dict[str, object]) -> str:
        ...
        if isinstance(pattern, str) and pattern == "^CED-[A-Z0-9-]+$":
            return "CED-PRJ-TEST-SCN-001"
        if isinstance(pattern, str) and pattern == "^EVD-[0-9]{3,}$":
            return "EVD-001"
        if isinstance(pattern, str) and pattern == "^CQP-[A-Z0-9-]+@v[0-9]{3,}$":
            return "CQP-PRJ-TEST-SCN-001@v001"
        if isinstance(pattern, str) and pattern == "^CCR-[A-Z0-9-]+@v[0-9]{3,}$":
        ...
        if isinstance(pattern, str) and pattern == "^ASP-[A-Z0-9-]+$":
            return "ASP-TEST-001"
        if isinstance(pattern, str) and pattern == "^CC-[A-Z0-9-]+$":
            return "CC-TEST-001"
        if isinstance(pattern, str) and pattern == "^CCS-[A-Z0-9-]+@v[0-9]{3,}$":
            return "CCS-TEST@v001"
        if isinstance(pattern, str) and pattern == "^CCS-ACTING-MASTER@v[0-9]{3,}$":
