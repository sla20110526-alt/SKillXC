...
- 人物资产画幅按排版需要，不跟随成片画幅。
- 场景默认使用能读取空间深度的3/4斜角母图；只有用户明确要求时才切换为剧照式母图，否则漂亮构图作为英雄视图而不覆盖母图。母图、场景状态、每张预制/专项/英雄视图和光影状态使用不同正式资产 ID，按精确父版本逐层依赖；空间圣经、锚点表、视图投影表和光源地图分别作为任务成果由任务成果索引追踪，不作为图片资产登记。
