...
6. `style-lock-director`：依次执行“项目风格意图基线 → 四卡专业基线链 → P0风格测试与诊断 → 组合快照与批量放行”。导演、美术、摄影 Profile 只在各自阶段询问和读取；责任 Skill 按“项目风格基线 → 美术 LookDev → 全片摄影 → 项目灯光”唯一写入长期基线，`creative-control-versioning` 在专用对话中管理草案与原子生效。四卡均当前有效后，选择最少高风险正式资产槽位，并用绑定单个测试ID与预留ID的临时短切片走正常 GPT Image 2 生产验证；四卡生效和测试通过都不等于登记，也不单独放行批量生产。最终风格包只组合四张精确版本、测试评审和三个资产短执行切片，不反向改写上游，也不成为第五张长期基线。
...
- 风格批量放行必须同时满足四张长期基线当前有效且依赖闭合、必做P0测试通过、没有未决专业冲突或顾问硬事实缺口。任一来源卡被替代或失效后，旧风格包快照不得用于新任务。
