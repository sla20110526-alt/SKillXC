...
| `video-prompt-production` | 是否使用；单镜/镜头组/返修模式；分镜/剪辑/摄影/表演/灯光/声音/VFX/就绪版本；本次平台能力 | `VRM`参考顺序/唯一用途数据；`VPR`单镜或镜头组正式Prompt；轻量检查点 | 用户手工复制到外部Seedance；按需进度同步或QC |
...
| `video-qc-review` | 是否使用；即时诊断/项目复盘/正式记录；分镜/镜头/生成单元/VPR/VRM版本；视频/截图/文字反馈证据边界；记录授权原文 | 默认只分析；逐问题给可观察差异、八类问题、主根因层、唯一责任节点和最小复核；明确保存后写`QCR`、QC表/卡与报告 |
